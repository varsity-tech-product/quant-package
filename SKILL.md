...
### ③ 部署币安实盘（日度调仓）
实盘有**两条 rail**：
- **③a Python 直连**（本节）：本地跑 `build_signal` 算权重，`quantkit.live` 直接签名打
  币安 REST。最轻、无需 Lean。
- **③b 经 gateway 的 Lean `binance_direct`**：把因子组合渲染成 Lean C# 策略
  （`quantkit.compose_csharp`，与回测同口径），由 `quantkit.live.gateway_launch` 起
  paper-runner，经 gateway 直连币安。需要本机装 Lean + paper-runner + gateway 插件；
  默认 `dry-run=true` 安全闸。细节见 `reference/lean_live_composer.md`，样例
  `examples/06_compose_live_csharp.py`。要求"实盘走 gateway 接口"时用这条。

③a 实盘**在本地跑 `build_signal`**，数据走 exchange-gateway（只用 1d；bars/feature=8778、
...
| `quantkit.compose_csharp` | 因子组合→Lean **实盘 C# 策略**渲染（vendored 拼接器；见 `reference/lean_live_composer.md`） |
